Shader "Custom/Cel Shade Baked Graphics" {
  Properties {
  ...
  SubShader {
    ...
    Pass {
      ...
      #define GRAPHIC_RENDERER_VERTEX_UV_1
      ...
      v2f_graphic_baked vert (appdata_graphic_baked v) {
        ...
        // Store Lit Amount in uv_1
        half3 lightDir = UnityWorldSpaceLightDir(v.vertex);
        half litAmount = 1;
        litAmount = max(0, (dot(UnityObjectToWorldDir(v.normal), lightDir) + 0.7) / 2);
        o.uv_1 = litAmount;
      ...
      //color *= abs(dot(normalize(i.normal.xyz), float3(0, 0, 1)));
        ...
        color *= celShadedColor(4, i.uv_1.x, i.color);
